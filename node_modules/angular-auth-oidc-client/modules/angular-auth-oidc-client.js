...
class OidcConfigService {
    ...
    load(configUrl) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const /** @type {?} */ response = yield fetch(configUrl);
                if (!response.ok) {
    ...
    load_using_stsServer(stsServer) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const /** @type {?} */ response = yield fetch(`${stsServer}/.well-known/openid-configuration`);
                if (!response.ok) {
                    throw new Error(response.statusText);
                }
                ...
                console.error(`OidcConfigService 'load_using_stsServer' threw an error on calling ${stsServer}`, err);
                this.onConfigurationLoaded.emit(false);
            }
        });
    }
    ...
    load_using_custom_stsServer(stsServer) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const /** @type {?} */ response = yield fetch(stsServer);
                if (!response.ok) {
                    throw new Error(response.statusText);
                }
                this.wellKnownEndpoints = yield response.json();
                ...
                console.error(`OidcConfigService 'load_using_custom_stsServer' threw an error on calling ${stsServer}`, err);
                this.onConfigurationLoaded.emit(false);
            }
        });
    }
...
class OidcSecurityValidation {
    ...
    validate_id_token_exp_not_expired(decoded_id_token, offsetSeconds) {
        const /** @type {?} */ tokenExpirationDate = this.tokenHelperService.getTokenExpirationDate(decoded_id_token);
    ...
    validate_id_token_aud(dataIdToken, aud) {
        if (dataIdToken.aud instanceof Array) {
            const /** @type {?} */ result = this.arrayHelperService.arraysEqual(dataIdToken.aud, aud);
            if (!result) {
    ...
    validate_signature_id_token(id_token, jwtkeys) {
        ...
        const /** @type {?} */ header_data = this.tokenHelperService.getHeaderFromToken(id_token, false);
...
class OidcSecurityCheckSession {
     ...
     * @param {?} zone
     */
    constructor(authConfiguration, oidcSecurityCommon, loggerService, zone) {
        this.authConfiguration = authConfiguration;
        this.oidcSecurityCommon = oidcSecurityCommon;
        this.loggerService = loggerService;
        this.zone = zone;
...
class OidcSecurityService {
     ...
     * @param {?} zone
     */
    constructor(platformId, oidcDataService, stateValidationService, authConfiguration, router, oidcSecurityCheckSession, oidcSecuritySilentRenew, oidcSecurityUserService, oidcSecurityCommon, oidcSecurityValidation, tokenHelperService, loggerService, zone) {
    ...
    authorize(urlHandler) {
        ...
        this.loggerService.logDebug('AuthorizedController created. local state: ' +
            this.oidcSecurityCommon.authStateControl);
        const /** @type {?} */ url = this.createAuthorizeUrl(this.authConfiguration.redirect_url, nonce, state, this.authWellKnownEndpoints.authorization_endpoint);
        if (urlHandler) {
            urlHandler(url);
        }
        else {
    ...
    refreshSession() {
        ...
        this.loggerService.logDebug('RefreshSession created. adding myautostate: ' +
            this.oidcSecurityCommon.authStateControl);
        const /** @type {?} */ url = this.createAuthorizeUrl(this.authConfiguration.silent_redirect_url, nonce, state, this.authWellKnownEndpoints.authorization_endpoint, 'none');
     ...
     * @return {?}
     */
    createAuthorizeUrl(redirect_url, nonce, state, authorization_endpoint, prompt) {
